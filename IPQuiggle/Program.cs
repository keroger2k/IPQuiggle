...
namespace IPQuiggle
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            /// List of IP Addresses on each line;  NO
            /// quotes.
            /// 
            /// Format:
            ///     10.1.1.1
            ///     10.1.1.1
            var ips =  File.ReadAllLines("c:\\tmp\\ips.txt");
            /// List of CIDRs; Unique CIDR on each line with NO
            /// quotes.
            /// 
            /// Format:
            ///     10.1.1.0/24
            ///     10.1.2.0/24
            var cidrs = File.ReadAllLines("c:\\tmp\\cidrs.txt");
            ...
            foreach (var ip in ips)
            {
                ...
                {
                    if (ipHelper.ip_inside_range(c, ip))
                    {
                        found = true;
                        Console.WriteLine(string.Format("IP:{0} is in CIDR: {1}", ip, c));
                    }
                }

                if(!found)
                {
                    Console.WriteLine(string.Format("Didn't find CIDR for IP: {0}", ip));
                }
            }
            Console.ReadLine();
        }
